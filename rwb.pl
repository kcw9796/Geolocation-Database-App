...
if ($action eq "base") { 
  ...
  print "<label><input type=\"checkbox\" style=\"margin-left:25px;\" id=\"cbox1\" value=\"first_checkbox\" checked> Committees - black</label>";
  print "<label><input type=\"checkbox\" style=\"margin-left:25px;\" id=\"cbox2\" value=\"second_checkbox\"> Candidates - white </label>";
  print "<label><input type=\"checkbox\" style=\"margin-left:25px;\" id=\"cbox3\" value=\"third_checkbox\"> Individuals - green</label>";
  print "<label><input type=\"checkbox\" style=\"margin-left:25px;\" id=\"cbox4\" value=\"fourth_checkbox\"> Opinions - purple</label><br>";

  my $str = Cycles();
  print $str;


  print "<br><button type=\"button\" onclick=\"BoxChange();\"> Update </button><br>";

  print "<div id=\"comm_money_data_target\" style=\"width:20\%;margin-top:5px;\"></div>";

  print "<div id=\"ind_money_data_target\" style=\"width:20\%;margin-top:5px;\"></div>";

  print "<div id=\"opinion_color_data_target\" style=\"width:20\%;margin-top:5px;\"></div>";
  #
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  if ($user eq "anon") {
    ...
    if (UserCan($user,"give-opinion-data")) {
      print "<p><a href=\"Javascript:GetCoords();\">Give Opinion Of Current Location</a></p>";
    }
...
if ($action eq "near") {
  ...
  if ($what{committees}) { 
    my ($str,$str2,$error) = Committees($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
  ...
  print "<div id=\"comm_money_data\">Nearby Committee Transactions: <br> $str2</div>";
    }
  }
  if ($what{candidates}) {
  ...
  if ($what{individuals}) {
    my ($str,$str2,$error) = Individuals($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
  ...
  print "<div id=\"ind_money_data\">Nearby Individual Transactions: <br> $str2</div>";
    }
  }
  if ($what{opinions}) {
    my ($str,$str2,$error) = Opinions($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
  ...
  print "<div id=\"opinion_color_data\">Nearby Opinion Colors: <br> $str2</div>";
    }
  }
}
...
if ($action eq "register") { 
  ...
  if (!CanRegister($email)) { 
    print h2('You cannot register anymore');
  } 
  else {
    if (!$run) { 
      print " <h1>Please Register Below</h1>
  ...
  h2('Invite User'),
    ...
    p,
      ...
      if (defined(param("email"))) { 
          my $e = param("email");
          my $subject = "you-have-been-invited";
          my $url = 'murphy.wot.eecs.northwestern.edu/~kcw507/rwb/rwb.pl?act=register\&email=' . $e . '\&referer=' . $user;
          my $data = 'Click Here: ' . $url;
          my $mail_status = `echo $data | mail -s $subject $e`;
          print "An invitation link has been sent to: $e as referred by $user\n";
          print "<script> alert(\"An invitation link has been sent to $e\"); </script>";
          my $error;
          $error = RegisterAdd($e);
          if ($error) { 
...
if ($action eq "give-opinion-data") { 
  if (!UserCan($user,"give-opinion-data")) { 
    ...
    if (!$run) { 
      ...
      print " <h1>Please assign a political color to your location</h1>
              <h3>Select red for republican or blue for democrat</h3>

              <form method=\"get\">
                <input type=\"radio\" name=\"color\" value=\"-1\">Red<br>
      ...
      if ($color == 1 or $color == -1) {
        # if color exists, add it to the rwb_opinons table 
        my $error;
        $error=ExecSQL($dbuser, $dbpasswd, "INSERT INTO rwb_opinions (submitter,color,latitude,longitude) VALUES (?,?,?,?)", undef,$user,$color,$lat,$long);
        if ($error) { 
          print "Can't add user because: $error";
        } 
        else {
          print "<h2>Your opinion has been added. Thank you for contributing. Please click return to go back.\n</h2>";
        }
      }
    }
  }
...
sub Cycles {
  ...
  my $checkval;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from CS339.committee_master");
  };
  ...
  foreach my $r (@rows) { 
      ...
      if ($cur eq "1112")
       { $checkval = "checked"; }
      $out.= "<label><input type=\"checkbox\" style=\"margin-left:10px;\" id=". $cur ." value=". $cur ." class=\"cycles\"". $checkval .">" . $cur . "</label>";
  }
...
sub Committees {
  ...
  eval { 
    ...
    @money = ExecSQL($dbuser, $dbpasswd, "select B.cmte_pty_affiliation,sum(A.transaction_amnt) from (select cmte_id,transaction_amnt,cycle from cs339.comm_to_cand union all select cmte_id,transaction_amnt,cycle from cs339.comm_to_comm)A, (select * from cs339.committee_master natural join cs339.cmte_id_to_geo) B where A.cmte_id = B.cmte_id and A.cycle = B.cycle and A.cycle in ($cycle) and B.latitude>? and B.latitude<? and B.longitude>? and B.longitude<? group by B.cmte_pty_affiliation",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) { 
    ...
    if ($format eq "table") { 
      return (MakeTable("committee_data","2D",
			...
			@rows),MakeTable("committee_money_data_table","2D",["affiliation","transaction amount
        "],@money),$@);
    } else {
      return (MakeRaw("committee_data","2D",@rows),MakeTable("committee_money_data_table","2D",["affiliation","transaction amount
        "],@money),$@);
    }
...
sub Individuals {
  ...
  eval { 
    ...
    @money = ExecSQL($dbuser, $dbpasswd, "select cmte_pty_affiliation,sum(transaction_amnt) from cs339.individual natural join cs339.ind_to_geo natural join cs339.committee_master where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? group by cmte_pty_affiliation",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) { 
    ...
    if ($format eq "table") { 
      return (MakeTable("individual_data", "2D",
			...
			@rows),MakeTable("individual_money_data_table","2D",["affiliation","transaction amount
        "],@money),$@);
    } else {
      return (MakeRaw("individual_data","2D",@rows),MakeTable("individual_money_data_table","2D",["affiliation","transaction amount
        "],@money),$@);
    }
  }
}
...
sub Opinions {
  ...
  eval { 
    ...
    @opinions = ExecSQL($dbuser, $dbpasswd, "select avg(color),stddev(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) { 
    ...
    if ($format eq "table") { 
      return (MakeTable("opinion_data","2D",
			["latitude", "longitude", "name", "city", "state", "zip", "employer", "amount"],
			@rows),MakeTable("opinion_color_data_table","2D",["Avg","St Dev"],@opinions), $@);
    } else {
      return (MakeRaw("opinion_data","2D",@rows),MakeTable("opinion_color_data_table","2D",["Avg","St Dev"],@opinions),$@);
    }
  }
}
...
sub RegisterAdd { 
  eval { ExecSQL($dbuser,$dbpasswd,
     "insert into rwb_register_list values (?)",undef,@_);};
  return $@;
}

sub RegisterDel { 
  eval { ExecSQL($dbuser,$dbpasswd,
     "delete from rwb_register_list where email = ?",undef,@_);};
  return $@;
}
...
sub CanRegister {
  my @col;
  eval {@col= ExecSQL($dbuser,$dbpasswd, "select count(*) from rwb_register_list where email=?","COL",@_);};
  if ($@) { 
    return 0;
  } else {
    return $col[0]>0;
...
sub MakeTable {
  ...
  if ((defined $headerlistref) || ($#list>=0)) {
    ...
    #
    $out="<table id=\"$id\"  style=\"color:white;\" border>";
    #
